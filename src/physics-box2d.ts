...
export class Box2dPhysics implements IPhysics {
    ...
    public applyImpulseToMarble(marbleId: number, impulse: { x: number, y: number }) {
        const marbleBody = this.getMarbleBodyById(marbleId);  // 구슬의 물리적 바디 가져오기
        if (marbleBody) {
            const impulseVector = new this.Box2D.b2Vec2(impulse.x, impulse.y);  // 충격 벡터 생성
            const marblePosition = marbleBody.GetWorldCenter();  // 구슬의 중심 좌표
            marbleBody.ApplyLinearImpulse(impulseVector, marblePosition, true);  // 구슬에 충격 적용
        }
    }
    applyForceToMarble(marbleId: number, force: { x: number, y: number }) {
        const marbleBody = this.getMarbleBodyById(marbleId);  // 구슬 ID로 물리 바디 찾기
        if (marbleBody) {
            const forceVector = new this.Box2D.b2Vec2(force.x, force.y);  // 힘 벡터 생성
            const marblePosition = marbleBody.GetWorldCenter();  // 구슬의 중심 좌표
            marbleBody.ApplyForce(forceVector, marblePosition, true);  // 힘 적용
        }
    }
    public  getMarbleBodyById(marbleId: number): Box2D.b2Body | undefined {
        // marbleMap 객체에서 구슬 ID에 해당하는 물리 바디를 반환
        return this.marbleMap[marbleId];
    }
